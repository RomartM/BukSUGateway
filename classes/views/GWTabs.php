...
class GWTabs
{
     ...
     * @param array $content_meta Usage: array("slug"=>"{content_label}", "content_id"=>"{content_id}")
    ...
    protected function nav(){
        ...
        foreach ($this->content as $data){
            $is_active = "";
            if( GWUtility::instance()->format_group_name($this->tab) === GWUtility::instance()->format_group_name($data[$this->content_meta['slug']])){
                $this->tab_index = $index;
                $is_active = "nav-tab-active";
            }
            echo "<a href=\"?page=" . $this->page_slug . "&" . $this->tab_name . "=". GWUtility::instance()->format_group_name($data[$this->content_meta['slug']])."\" class=\"nav-tab " . $is_active . "\">". $data[$this->content_meta['slug']] ."</a>";
            $index++;
        }
        echo '</nav>';
    }
     ...
     * Build Tab
     *
     * @param mixed $content_callback  For custom dynamic content if empty will use content_id as sub page path to include
     */
    public function build($content_callback){
        ...
        $content_id = $this->content[$index][$this->content_meta['content_id']];

        echo '<div class="tab-content">';
        if(empty($content_callback)){
            $dir_path = WP_GW_ROOT . '/' . $content_id;
            ...
            $content_callback($this->tab, $content_id);
        }
        echo '<div>';
    }
